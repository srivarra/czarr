...
impl for Delta / FixedScaleOffset.  The choice is a runtime knob —
...
# writes here; ``resolve_backend_for_filter`` reads it.  Per-instance
...
def resolve_backend_for_filter(
    ...
    *,
    ...
    Filter codecs (Shuffle / Delta / FixedScaleOffset) call this from
    their ``__post_init__``.  Precedence: per-instance ``backend=`` kwarg
    wins over the process-global override (set via
    :func:`czarr.configure_gpu`), which wins over the class default.

    Raises :class:`ValueError` when the choice picks a backend the codec
    does not support — silent fallback hides user mistakes.
    """
